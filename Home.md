...
Steeltoe 4.x is available for working with .NET 8 and newer.
Steeltoe 3.x is available for working with .NET Core 3.1 and newer.
Steeltoe 2.x is available for working with .NET Framework 4.5.2 and newer.
...
* [Documentation](https://github.com/SteeltoeOSS/Documentation) - The [Steeltoe.io](https://steeltoe.io) website
* [Initializr](https://start.steeltoe.io)
  ...
  * [NetCoreToolService](https://github.com/SteeltoeOSS/NetCoreToolService) - web service for creating projects generated by the .NET CLI with NetCoreToolTemplates
  * [NetCoreToolTemplates](https://github.com/SteeltoeOSS/NetCoreToolTemplates) - .NET templates, for use with Initializr, the .NET CLI and IDEs
...
* [Schema](https://github.com/SteeltoeOSS/Schema) - JSON schema aggregation for Steeltoe components
...
* **Release Candidate** (for example: rc1, rc2, ...) - Similar to a beta release. A release candidate contains a full feature set and is available to the community for finding any major issues before the GA version is released. There could still be source code changes to fix defects, changes to documentation, and test cases, but these releases are generally considered production-grade and (until a GA release is available) their usage are supported in production environments.
* **General Availability** (GA) - This is the full featured and fully tested release that is available to the public for use in production environments.
...
Maintenance branches are created after each major release (eg: 3.x) and minor branches may be created as needed for regressions and/or security issues.
